...
from stp3.losses import SpatialRegressionLoss, SegmentationLoss, HDmapLoss, DepthLoss, PixelFocalLoss
...
class TrainingModule(pl.LightningModule):
    def __init__(self, hparams):
        ...
        if self.cfg.SEMANTIC_SEG.HEATMAP.ENABLED:
            # heatmap prediction 
            # from Home : https://github.com/Robotmurlock/TNT-VectorNet-and-HOME-Trajectory-Forecasting
            self.losses_fn['heatmap'] = PixelFocalLoss()
            self.model.heatmap_weight = nn.Parameter(torch.tensor(0.0), requires_grad=True)
            self.metric_heatmap_val = softIoU(2)
        ...
        # Pedestrian segmentation # we use this head to predict goal area semantic 
        ...
        # for centerness metric 
        self.metric_centerness_val = softIoU(2)
        ####
    ...
    def shared_step(self, batch, is_train):
        ...
        # Forward pass
        states, output = self.model(
            image, intrinsics, extrinsics, future_egomotion# , labels
        )
        ...
        if is_train:
            ...
            # heatmap 
            if self.cfg.SEMANTIC_SEG.HEATMAP.ENABLED:    
                loss['heatmap'] = self.losses_fn['heatmap'](
                    output['heatmap'], labels['heatmap'], 
                )
            ...
            if self.cfg.PLANNING.ENABLED:
                ...
                pl_loss, final_traj = self.model.planning(states, labels['gt_trajectory'][:, 1:, :2], target_points)
                #print('train', final_traj.shape)
                ...
                output = {**output, 'selected_traj': torch.cat(
                    [torch.zeros((B, 1, 2), device=final_traj.device), final_traj], dim=1)}
            else:
        ...
        # Metrics
            ...
            self.metric_centerness_val(pred_centerness[:, n_present - 1:], labels['centerness'][:, n_present - 1:])

            
            
            # heatmap metric 
            if self.cfg.SEMANTIC_SEG.HEATMAP.ENABLED:
                heatmap_prediction = output['heatmap']
                self.metric_heatmap_val(heatmap_prediction[:,:], labels['heatmap'][:, :])
            else:
                heatmap_prediction = torch.zeros_like(seg_prediction)
            ...
            if self.cfg.PLANNING.ENABLED:
                ...
                '''
                _, final_traj = self.model.planning(states, labels['gt_trajectory'][:, 1:, :2], target_points)
                #print('val', final_traj.shape)
                occupancy = torch.logical_or(labels['segmentation'][:, n_present:].squeeze(2),
                                             labels['pedestrian'][:, n_present:].squeeze(2))
                self.metric_planning_val(final_traj, labels['gt_trajectory'][:, 1:, :2], occupancy)
                output = {**output,
                          'selected_traj': torch.cat([torch.zeros((B, 1, 2), device=final_traj.device), final_traj],
                                                     dim=1)}
            else:
                output = {**output, 'selected_traj': labels['gt_trajectory']}
    ...
    def prepare_future_labels(self, batch):
        ...
        #     mode='nearest', spatial_extent=self.spatial_extent,
        # ).contiguous()
        # labels['hdmap_all'] = torch.cat([hdmap_labels_past, hdmap_labels], dim=1)
        ...
        if self.cfg.SEMANTIC_SEG.HEATMAP.ENABLED:
            # heatmap label 
            # warp heatmap labels to present's reference frame
            heatmap_labels = batch['heatmap']
            labels['heatmap'] = heatmap_labels[:, self.model.receptive_field - 1].float().contiguous() # since heatmap label is 0 to 1 (float number )
            
        # only predict one frame 
        ###################################################################
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.cfg.SEMANTIC_SEG.HEATMAP.ENABLED:
            scores = self.metric_heatmap_val.compute()
            self.log('step_val_heatmap_iou_dynamic', scores[1])
    ...
    def shared_epoch_end(self, step_outputs, is_train):
        if not is_train:
            ...
            # metric_centerness_val
            
            scores = self.metric_centerness_val.compute()
            self.logger.experiment.add_scalar('epoch_val_all_centerness_iou', scores[1],
                                            global_step=self.training_step_count)
            scores = self.metric_centerness_val.reset()
        
            
            if self.cfg.SEMANTIC_SEG.HEATMAP.ENABLED:
                # heatmap
                scores = self.metric_heatmap_val.compute()
                self.logger.experiment.add_scalar('epoch_val_all_heatmap_iou_dynamic', scores[1],
                                                global_step=self.training_step_count)
                scores = self.metric_heatmap_val.reset()
