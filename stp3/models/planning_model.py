...
class Planning(nn.Module):
    ...
    def __init__(self, cfg, feature_channel, gru_input_size=6, gru_state_size=256):
        ...
        '''
    def __init__(self, gru_state_size):
        super().__init__()
        ...
        self.output = nn.Linear(gru_state_size, 3)
    ...
    """
    def forward(self, z, ego_waypoint, target_point):
        #z = self.MLP(z)
        ...
        for _ in range(4):
            ...
            '''
            if self.config.gru_concat_target_point:
                x_in = torch.cat([x, target_point], dim=1)
            else:
                x_in = x
            '''
