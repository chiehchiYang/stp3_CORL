...
from stp3.models.home_decoder import HeatmapOutputDecoder, HeatmapEncoder
...
class STP3(nn.Module):
    def __init__(self, cfg):
        ...
        if self.cfg.SEMANTIC_SEG.HEATMAP.ENABLED:
            #################### heatmap encoder and decoder #########################################
            self.heatmap_encoder = HeatmapEncoder((64, 200, 200)) # encoder's input shape
            self.heatmap_decoder = HeatmapOutputDecoder((64+512, 200, 200))  # decoder's input shape 
            ##########################################################################################
        ...
        if self.cfg.PLANNING.ENABLED:
            self.planning = Planning(gru_state_size=self.cfg.PLANNING.GRU_STATE_SIZE)
    ...
    def forward(self, image, intrinsics, extrinsics, future_egomotion):
        ...
        # print(labels['hdmap_all'].shape)
        
        # print(labels['hdmap_all'][0,0,0,:,:])
        # print(torch.nonzero(labels['hdmap_all'][0,0,0,:,:]))
        
        # b, t, c, w, h = states.shape
        # for i in range(b):
        #     for j in range(t):
        #         lane_nonzero = torch.nonzero(labels['hdmap_all'][i,j,0,:,:])
        #         # road_nonzero = torch.nonzero(labels['hdmap_all'][i,j,1,:,:])
                
        #         for k in range(c):
        #             # i, j, k
        #             states[i, j, k, :, :][lane_nonzero] = 1.0

        # 4, 3
        # torch.Size([4, 3, 64, 200, 200])
        # torch.Size([4, 3, 2, 200, 200])
        ...
        if self.n_future > 0:
            ...
            ########## predict heatmap output ##########
            # states shape: b * s * c * h * w
            # current state: b * 1 * c * h * w  ==> b*1*64*200*200
            # encoder_feature: b*1*512*200*200
            # merge_feature: b*1*576*200*200
            ###########################################

            if self.cfg.SEMANTIC_SEG.HEATMAP.ENABLED:
                current_state = states[:,2,:,:,:]
                encoder_feature = self.heatmap_encoder(current_state)
                merge_feature = torch.concat([current_state, encoder_feature], dim=1)
                heatmap_output = self.heatmap_decoder(merge_feature)  # heatmap_output: b*1*200*200

            # predict BEV outputs
            bev_output = self.decoder(states)
            states = self.global_pool(states) 
            states = states[:,self.receptive_field-1,:,:,:]
    ...
    def projection_to_birds_eye_view(self, x, geometry, future_egomotion):
        ...
        #print('In projection_to_birds_eye_view', x.shape)
    ...
    def calculate_birds_eye_view_features(self, x, intrinsics, extrinsics, future_egomotion):
        ...
        # geometry torch.Size([24, 4, 48, 16, 32, 3])
        # torch.Size([24, 4, 48, 4, 32, 64])
        # torch.Size([8, 3, 4, 48, 4, 32, 64])
