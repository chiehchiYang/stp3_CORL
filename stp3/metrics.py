...
class softIoU(Metric):
    ...
    def update(self, prediction: torch.Tensor, target: torch.Tensor):
        ...
        pred = torch.where(prediction > 0.2, one, zero)
        tar = torch.where(target > 0.2, one, zero)
        
        
        tps, fps, _, fns, sups = stat_scores_multiple_classes(pred, tar, self.n_classes)
        self.true_positive += tps
        self.false_positive += fps
        self.false_negative += fns
        self.support += sups
    ...
    def compute(self):
        ...
        return scores, tp, fp, fn #reduce(scores, reduction=self.reduction)