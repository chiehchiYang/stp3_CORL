...
def visualise_output(labels, output, cfg):
    ...
    for t in range(sequence_length):
        ...
        if cfg.SEMANTIC_SEG.HEATMAP.ENABLED:
            # heatmap Ground truth
            heatmap = labels['heatmap'].cpu().numpy()
            heatmap_plot = heatmap[0][0][::-1, ::-1]
            plt.figure(figsize=(2.6, 2.6))
            plt.imshow(heatmap_plot, cmap='Blues' )
            buffer = BytesIO()
            plt.axis('off')
            plt.tight_layout()
            plt.savefig(buffer, format='png', bbox_inches='tight', pad_inches=0.0)
            heatmap_plot = np.asarray(Image.open(buffer).convert('RGB')) # 200, 200, 3 
            plt.close()
            buffer.close()
            
            h, w, _ = heatmap_plot.shape
            offset_h = int( (h-200)/2 )
            heatmap_gt_plot = heatmap_plot[(offset_h):(h-offset_h), (w-200):]
            heatmap_gt_plot = heatmap_gt_plot[:200, :200]
        else: 
            heatmap_gt_plot = np.zeros_like(semantic_plot)
        ...
        out_t.append(np.concatenate([instance_plot, future_flow_plot,
                                     semantic_plot, center_plot, offset_plot, pedestrian_plot, planning_plot, heatmap_gt_plot], axis=0))
        ...
        if cfg.SEMANTIC_SEG.HEATMAP.ENABLED:
            # heatmap prediction 
            # Predictionsmas
            heatmap = output['heatmap'].detach().cpu().numpy()
            heatmap_plot = heatmap[0][0][::-1, ::-1]
            ...
            h, w, _ = heatmap_plot.shape
            offset_h = int( (h-200)/2 )
            heatmap_prediction_plot = heatmap_plot[(offset_h):(h-offset_h), (w-200):]
        else: 
            heatmap_prediction_plot = np.zeros_like(semantic_plot)
        ...
        out_t.append(np.concatenate([instance_plot, future_flow_plot,
                                     semantic_plot, center_plot, offset_plot, pedestrian_plot, planning_plot, heatmap_prediction_plot], axis=0))
